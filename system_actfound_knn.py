...
class ActFoundKNNRegressor(RegressorBase):
    ...
    def inner_loop_knn(self, data_batch_knn, x_task, y_task, assay_idx, split, is_training_phase, epoch, num_steps):
        xs_knn, ys_knn, splits_knn, _, assay_weight_knn, _ = data_batch_knn
        task_losses = []
    ...
    def find_knn_batch(self, data_batch):
        ...
        knn_assay_idx = [self.train_assay_names_all[x] for x in knn_idx]
        if len(knn_assay_idx) > 0:
            ...
            data_batch_knn = [x for x in self.assay_dataset.get_knn_assays(knn_assay_idx, top16_weight * 0.2)]
        else:
            data_batch_knn = None
    ...
    def run_validation_iter_knnmaml(self, data_batch):
        ...
        sup_num = torch.sum(split)
        if data_batch_knn is not None and sup_num >= 12:
            print(f"identified {len(data_batch_knn[0])} knn assays")
            print("identified assays and weights:", [(a,b/0.2) for a,b in zip(data_batch_knn[3], data_batch_knn[4])])
    ...
    def prepare_knn_maml(self, assay_dataset):
        self.assay_dataset = assay_dataset
        self.train_assay_names_all = json.load(open(self.args.train_assay_idxes, "r"))
    ...
    def run_predict(self, x_task, y_task, split):
        data_batch = [x_task], [y_task], [split], [0], [1], ["test"]
        losses, per_task_target_preds, final_weights, per_task_metrics = self.run_validation_iter(data_batch)
